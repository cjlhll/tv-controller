...
assert.equal(logic.todayUnlockMinutes(watch4, after), 8)
assert.equal(watch4.unlockSessionStart, midnight)

const watch5 = logic.applyRegister(null, { name: '息屏' }, now + 80000)
logic.applyBind(watch5, now + 80000)
logic.applyApprove(watch5, 60, t0)
logic.applyScreenPresence(watch5, { screenOn: false }, t0 + 10 * 60 * 1000)
assert.equal(logic.todayWatchMinutes(watch5, t0 + 20 * 60 * 1000), 10)
assert.equal(logic.todayUnlockMinutes(watch5, t0 + 20 * 60 * 1000), 20)
assert.equal(watch5.watchSessionStart, 0)
assert.ok(watch5.unlockSessionStart > 0)
logic.applyScreenPresence(watch5, { screenOn: true }, t0 + 25 * 60 * 1000)
assert.equal(logic.todayWatchMinutes(watch5, t0 + 30 * 60 * 1000), 15)
assert.equal(logic.todayUnlockMinutes(watch5, t0 + 30 * 60 * 1000), 30)
logic.applyLock(watch5, t0 + 30 * 60 * 1000)
assert.equal(logic.todayWatchMinutes(watch5, t0 + 40 * 60 * 1000), 15)
assert.equal(logic.todayUnlockMinutes(watch5, t0 + 40 * 60 * 1000), 30)
assert.equal(logic.publicDevice(watch5, t0 + 40 * 60 * 1000).todayUnlockMin, 30)
assert.equal(logic.publicDevice(watch5, t0 + 40 * 60 * 1000).todayWatchMin, 15)

const watch6 = logic.applyRegister(null, { name: '旧数据' }, now + 90000)
logic.applyBind(watch6, now + 90000)
watch6.status = 'unlocked'
watch6.unlockUntil = t0 + 60 * 60 * 1000
watch6.todayWatchMs = 12 * 60 * 1000
watch6.todayWatchDate = logic.shanghaiDate(t0)
watch6.watchSessionStart = t0
delete watch6.todayUnlockMs
delete watch6.unlockSessionStart
logic.rollTodayWatch(watch6, t0 + 8 * 60 * 1000)
assert.equal(watch6.todayUnlockMs, 12 * 60 * 1000)
assert.equal(watch6.unlockSessionStart, t0)
assert.equal(logic.todayUnlockMinutes(watch6, t0 + 8 * 60 * 1000), 20)
...
assert.equal(logic.publicDevice(device, now).allowUninstall, false)
assert.equal(logic.publicDevice(device, now).deviceOwner, undefined)
const allowOff = logic.applySetAllowUninstall(device, true, now + 72000)
assert.equal(allowOff.allowUninstall, true)
assert.equal(device.allowUninstall, true)
assert.equal(logic.publicDevice(device, now + 72000).allowUninstall, true)
logic.applySetAllowUninstall(device, 'false', now + 73000)
assert.equal(device.allowUninstall, false)
logic.applyCapabilities(device, { deviceOwner: true })
assert.equal(device.deviceOwner, true)
assert.equal(logic.publicDevice(device, now + 73000).deviceOwner, true)
const ownerFresh = logic.applyRegister(null, { name: 'Owner机', deviceOwner: true }, now + 74000)
assert.equal(ownerFresh.deviceOwner, true)
assert.equal(ownerFresh.allowUninstall, false)
